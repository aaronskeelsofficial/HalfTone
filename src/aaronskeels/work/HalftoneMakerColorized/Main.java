...
//						System.out.println(shift + " : " + x + " : " + toleranceLocationSizePx);
				...
				//Handle colorization
				BufferedImage colorizeImage = new BufferedImage(PatternImage.getWidth(), PatternImage.getHeight(), BufferedImage.TYPE_INT_ARGB);
				Graphics2D colorG2D = colorizeImage.createGraphics();
				colorG2D.drawImage(PatternImage, 0, 0, null, frame);
				colorG2D.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_IN, 0.5f));
				colorG2D.setColor(new Color(TargetImage.getRGB(x, y)));
				colorG2D.fillRect(0, 0, colorizeImage.getWidth(), colorizeImage.getHeight());
				colorG2D.dispose();
				
				//Handle Random Rotation
				BufferedImage imageToDraw = colorizeImage;
				if (doRandomizeRotation) {
					imageToDraw = rotateImage(colorizeImage, rand.nextDouble()*2*Math.PI);
				}
